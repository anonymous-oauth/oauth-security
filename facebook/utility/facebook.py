...
# Get the OAuth flow for the given domain
def get_domain_flow(domain):
    facebook = get_facebook(domain)
    return get_flow(facebook.get("authorization_url"))
...
# Get the sdk for the domain
def get_sdk(domain):
    facebook = get_facebook(domain)
    url = get_facebook_url(facebook.get("authorization_url"))
    return get_parameter(url, "sdk")
