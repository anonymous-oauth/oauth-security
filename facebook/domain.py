...
from utility.facebook import is_code_flow, get_facebook_url, get_sdk
...
class FacebookDomains(Domains):
    ...
    MARKER = Query().attacker_marker & (Query().attacker_marker != None) \
             & Query().victim_marker & (Query().victim_marker != None)
    ...
    IMPLICIT_DOMAINS = BASE & AUTH_URL & ~CODE_FLOW & ~AUTH_ERR & MARKER
    DOMAINS = BASE & AUTH_URL & ~AUTH_ERR & MARKER
    ...
    def get_implicit_all(self):
        return self.search(Query().idps.any(self.BASE & self.AUTH_URL & ~self.CODE_FLOW))

    def get_attack_implicit(self, state=None):
        if state is None:
            domains = self.search(Query().idps.any(self.IMPLICIT_DOMAINS))
        else:
            condition = ~self.EMPTY_STATE if state else self.EMPTY_STATE
            domains = self.search(Query().idps.any(self.IMPLICIT_DOMAINS & condition))
        domains = [x for x in domains if get_sdk(x) != "joey"]
        return domains

    def get_js_sdk(self):
        domains = self.search(Query().idps.any(self.IMPLICIT_DOMAINS))
        return [x for x in domains if get_sdk(x) == "joey"]
    ...
    def get_authorization_errors(self):
        return self.search(Query().idps.any(self.BASE & self.AUTH_URL & self.AUTH_ERR))

    def get_login_incomplete(self):
        return self.search(Query().idps.any(self.BASE & self.AUTH_URL & ~self.AUTH_ERR & ~self.MARKER))
    ...
    def get_domains(self, state=None):
        code = self.get_attack_domains(state=state)
        implicit = self.get_attack_implicit(state=state)
        return code + implicit
