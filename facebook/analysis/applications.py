...
def print_information(domains, limit=6):
    vulnerable_domains = [x["domain"] for x in get_vulnerable(domains)]
    all_domains, vulnerable = defaultdict(int), defaultdict(int)

    for domain in applications.all():
        category = domain["application"].get("category")
        all_domains[category] += 1
        if domain["domain"] in vulnerable_domains:
            vulnerable[category] += 1

    labels = [x[0] for x in sorted(all_domains.items(), key=lambda kv: kv[1], reverse=True)][:limit-1]
    values1 = [all_domains[x] for x in labels]
    values2 = [vulnerable[x] for x in labels]

    # Include the remaining data
    labels.append("Other")
    values1.append(len(domains) - sum(values1))
...
def main():
    domains = database.get_domains(state=None)
    # get_facebook_applications(domains)
    print_information(domains)
