...
from utility.facebook import get_flow, is_facebook_login
...
class FacebookTest(TestBase):
    ...
    def setUp(self):
        super().setUp()
        self.database = FacebookDomains("data/facebook")
    ...
    def attack(self, url):
        ...
        try:
            ...
            # Reach the marker page
            error = facebook.reach_marker_page(self.domain, long_wait=long_wait)
            if save_data:
    ...
    @parameterized.expand(
        test_domains(db.get_marker_url()),
        skip_on_empty=True
    )
    ...
    @parameterized.expand(
        test_domains(db.get_login_domains()),
        skip_on_empty=True
    )
    ...
    @parameterized.expand(
        ...
        test_domains(db.get_attack_domains(state=False)),
        skip_on_empty=True
    )
    ...
    @parameterized.expand(
        test_domains(db.get_attack_domains(state=True)),
        skip_on_empty=True
    )
